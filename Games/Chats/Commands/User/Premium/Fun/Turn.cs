namespace WibboEmulator.Games.Chats.Commands.User.Premium.Fun;
...
internal sealed class Turn : IChatCommand
{
    public void Execute(GameClient session, Room room, RoomUser userRoom, string[] parameters)
    {
        ...
        userRoom.OnChat(string.Format(LanguageManager.TryGetValue("cmd.turn.chat", session.Language), targetUser.Username));
        targetUser.OnChat(string.Format(LanguageManager.TryGetValue("cmd.turn.chat.target", session.Language), userRoom.Username));

        targetUser.ApplyEffect(561, true);
