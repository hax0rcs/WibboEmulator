...
public readonly struct SquarePoint(RoomUser user, int x, int y, int targetX, int targetY, byte squareData, bool isOverride, bool allowWalkthrough, byte squareDataUser)
{
    ...
    {
        var sqItems = user.Room.GameMap.GetCoordinatedItems(new Point(x, y));
        var flag = sqItems.Any(sqItem => sqItem.ItemData.InteractionType == InteractionType.PILE_WALK);
        ...
        if (this._lastStep)
        {
            return isOverride || squareData == 3 || squareData == 1 || flag;
        }

        return isOverride || squareData == 1 || flag;
    }
