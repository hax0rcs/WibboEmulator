...
public class RoomUserManager(Room room)
{
    ...
    public event EventHandler<UserIdleEventArgs> OnUserIdle;
    public event EventHandler<UserActionEventArgs> OnUserAction;

    public void UserEnter(RoomUser thisUser) => this.OnUserEnter?.Invoke(thisUser, new());

    public void UserAction(RoomUser thisUser, int action, int value = -1) =>
        this.OnUserAction?.Invoke(thisUser, new UserActionEventArgs(action, value));
    ...
    public void OnCycle(ref int idleCount)
    {
        ...
        foreach (var user in this.UserList.OrderBy(a => Guid.NewGuid()).ToList())
        {
            ...
            if (user.IsAsleep)
            {
                this.OnUserIdle?.Invoke(null, new UserIdleEventArgs(user, user.IdleTime));
            }
    ...
    private void CalculatePath(RoomUser user)
    {
        ...
        var nextStep = DreamPathfinder.GetNextStep(user, user.X, user.Y, user.GoalX, user.GoalY, gameMap.MapGame, gameMap.ItemHeightMap, gameMap.UserOnMap, gameMap.SquareTaking, gameMap.Model.MapSizeX, gameMap.Model.MapSizeY, user.AllowOverride, user.OverrideHeight, gameMap.DiagonalEnabled, room.RoomData.AllowWalkthrough, gameMap.ObliqueDisable);
        if (user.WalkSpeed)
        {
            nextStep = DreamPathfinder.GetNextStep(user, nextStep.X, nextStep.Y, user.GoalX, user.GoalY, gameMap.MapGame, gameMap.ItemHeightMap, gameMap.UserOnMap, gameMap.SquareTaking, gameMap.Model.MapSizeX, gameMap.Model.MapSizeY, user.AllowOverride, user.OverrideHeight, gameMap.DiagonalEnabled, room.RoomData.AllowWalkthrough, gameMap.ObliqueDisable);
        }
    ...
    private void HandleSetMovement(SquarePoint nextStep, RoomUser user)
    {
        ...
        foreach (var walkMagic in room.GameMap.GetCoordinatedItems(new Point(nextX, nextY)).Where(walkMagic => walkMagic.ItemData.InteractionType == InteractionType.PILE_WALK))
        {
            nextZ = walkMagic.Z;
        }
