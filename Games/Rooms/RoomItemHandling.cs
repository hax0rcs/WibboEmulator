...
public class RoomItemHandling(Room room)
{
    ...
    public List<Item> RemoveAllFurnitureToInventory(GameClient session)
    {
        ...
        room.SendPacket(room.GameMap.Model.SerializedHeightmap);
    ...
    public bool SetFloorItem(GameClient session, Item item, int newX, int newY, int newRot, bool newItem, bool onRoller, bool sendMessage, double z = 0.0)
    {
        ...
        else
        {
            foreach (var roomItem in itemsComplete)
            {
                ...
                if (tileMove || tileCopy)
                {
                    var incrementHeight = buildToolEnable ? buildToolHeight : 0.0;
                    pZ = item.Z + incrementHeight;
                    break;
                }
        ...
        if (!onRoller)
        {
            foreach (var roomItem in itemsComplete)
            {
                if (roomItem != null && roomItem.Id != item.Id && roomItem.ItemData != null && !roomItem.ItemData.Stackable && !buildToolEnable && !pileMagic && !pileWalk && !buildToolStackHeight)
                {
                    if (needsReAdd)
                    {
                        this.UpdateItem(item);
