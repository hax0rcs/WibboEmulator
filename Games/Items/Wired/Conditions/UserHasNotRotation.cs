...
public class UserHasNotRotation : WiredConditionBase, IWiredCondition, IWired
{
    public UserHasNotRotation(Item item, Room room) : base(item, room, (int)WiredConditionType.ACTOR_COMPARE_ROTATION) => this.DefaultIntParams(0, 0);
    ...
    public bool AllowsExecution(RoomUser user, Item item)
    {
        if (user != null)
        {
            ...
            if (!headOnly)
            {
                return user.RotBody != rot;
            }

            return user.RotHead != rot;
        }
