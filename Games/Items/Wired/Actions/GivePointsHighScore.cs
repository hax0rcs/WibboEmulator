    ...
    public override bool OnCycle(RoomUser user, Item item)
    {
        ...
        foreach (var highScoreItem in this.Items)
        {
                ...
                highScoreItem.Data.InteractionType == InteractionType.HIGH_SCORE_POINTS))
            {
                ...
                {
                    case HighScoreOperatorAction.Addition:
                        scores[user.Username] = score <= int.MaxValue - valueInt ? score + valueInt : valueInt;
                        break;
                    case HighScoreOperatorAction.Subtraction:
                        scores[user.Username] = score >= int.MinValue + valueInt ? score - valueInt : valueInt;
                        break;
                    case HighScoreOperatorAction.Multiplication:
                        if (valueInt > 0)
                        {
                            scores[user.Username] = (score > int.MaxValue / valueInt) ? int.MaxValue : score * valueInt;
                        }
                        else if (valueInt < 0)
                        {
                    ...
                    case HighScoreOperatorAction.Division:
                        scores[user.Username] = valueInt != 0 && score != 0 ? (int)Math.Round((double)score / valueInt) : 0;
                        break;
                    case HighScoreOperatorAction.Modulo:
                        scores[user.Username] = valueInt != 0 && score != 0 ? score % valueInt : 0;
                        break;
                    case HighScoreOperatorAction.Remplace:
                        scores[user.Username] = valueInt;
                        break;
                    case HighScoreOperatorAction.Delete:
                        _ = scores.Remove(user.Username);
                        break;
                }
