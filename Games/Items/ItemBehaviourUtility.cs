        ...
        switch (itemData.InteractionType)
        {
            ...
            case InteractionType.TERMINAL:
                if (!string.IsNullOrEmpty(item.ExtraData))
                {
                    ...
                    var terminalData = "state" + Convert.ToChar(9) + "0" + Convert.ToChar(9) + item.ExtraData;

                    terminalData = terminalData.Replace('=', Convert.ToChar(9));
                    message.WriteInteger(terminalData.Split(Convert.ToChar(9)).Length / 2);

                    for (var i = 0; i <= terminalData.Split(Convert.ToChar(9)).Length - 1; i++)
                    {
                        var data = terminalData.Split(Convert.ToChar(9))[i];
                        message.WriteString(data);
                    }
                }
                else
