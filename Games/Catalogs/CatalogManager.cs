...
public static class CatalogManager
{
    ...
    private static readonly Dictionary<int, CatalogBuildersPage> BuildersPages = [];
    ...
    private static readonly Dictionary<int, Dictionary<int, CatalogBuildersItem>> BuildersItems = [];
    ...
    private static readonly Dictionary<int, int> ItemsBuildersPage = [];
    ...
    public static void Initialize(IDbConnection dbClient)
    {
        ...
        if (ItemsBuildersPage.Count > 0)
        {
            ItemsBuildersPage.Clear();
        }
        ...
        if (catalogItemList.Count != 0)
        {
            ...
            var catalogBuildersItemList = CatalogBuildersItemDao.GetAll(dbClient);
            if (catalogBuildersItemList.Count != 0)
            {
                foreach (var catalogItem in catalogBuildersItemList)
                {
                    if (catalogItem.Amount <= 0)
                    {
                        continue;
                    ...
                    if (!ItemManager.GetItem(baseId, out var data))
                    {
                        Console.WriteLine("Couldn't load Catalog Builders Item " + itemId + ", no furniture record found.");
                        continue;
                    }
                    ...
                    if (!BuildersItems.TryGetValue(pageId, out var _))
                    {
                        BuildersItems.Add(pageId, []);
                    }

                    BuildersItems[pageId].Add(catalogItem.Id, new CatalogBuildersItem(catalogItem.Id, catalogItem.ItemId,
                        data, catalogItem.CatalogName, catalogItem.PageId, catalogItem.CostCredits,
                        catalogItem.CostPixels, catalogItem.CostDiamonds, catalogItem.CostLimitCoins,
                        catalogItem.Amount, catalogItem.LimitedSells,
                        catalogItem.LimitedStack, catalogItem.OfferActive,
                    ...
                    ItemsBuildersPage.Add(catalogItem.Id, pageId);
                }

                Console.WriteLine($"Catalog Manager -> BUILDERS PAGES LOAD >> {ItemsBuildersPage.Count}");
                Console.WriteLine($"Catalog Manager -> BUILDERS ITEMS LOAD >> {catalogBuildersItemList.Count}");
            }

            var catalogBuildersPageList = CatalogBuildersPageDao.GetAll(dbClient);
            if (catalogBuildersPageList.Count != 0)
            {
                foreach (var catalogPage in catalogBuildersPageList)
                {
                    BuildersPages.Add(catalogPage.Id, new CatalogBuildersPage(catalogPage.Id, catalogPage.ParentId, catalogPage.Enabled, catalogPage.Caption,
                        ...
                        BuildersItems.TryGetValue(catalogPage.Id, out var value) ? value : []));
                }

                Console.WriteLine($"Load Builders Clube Pages " + BuildersPages.Count);
            }
    ...
    public static CatalogBuildersItem FindBuildersItem(int itemId, User user)
    {
        if (!ItemsBuildersPage.TryGetValue(itemId, out var pageId))
        {
            return null;
        }

        if (!BuildersPages.TryGetValue(pageId, out var page))
        {
            return null;
        }
    ...
    public static bool TryGetBCPage(int pageId, out CatalogBuildersPage page) => BuildersPages.TryGetValue(pageId, out page);

    public static ICollection<CatalogPage> Pages => CatalogPages.Values;

    public static ICollection<CatalogBuildersPage> BCPages => BuildersPages.Values;
