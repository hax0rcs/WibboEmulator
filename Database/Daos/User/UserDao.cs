...
internal sealed class UserDao
{
    ...
    internal static void UpdateAvatarImage(IDbConnection dbClient, int userId, string avatarKey) => dbClient.Execute(
        "UPDATE user SET discord_avatar = @Avatar WHERE id = @UserId",
        new { Avatar = avatarKey, UserId = userId });

    internal static void UpdateBannerImage(IDbConnection dbClient, int userId, string bannerKey) => dbClient.Execute(
        "UPDATE user SET discord_banner = @Banner WHERE id = @UserId",
        new { Banner = bannerKey, UserId = userId });
