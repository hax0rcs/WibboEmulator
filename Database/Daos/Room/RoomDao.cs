...
internal sealed class RoomDao
{
    ...
    internal static void UpdateTileFlow(IDbConnection dbClient, int roomId, bool enabled) => dbClient.Execute(
        "UPDATE room SET tile_flow = @Enabled WHERE id = @RoomId LIMIT 1",
        new { Enabled = enabled ? 1 : 0, RoomId = roomId });
    ...
    internal static List<RoomEntity> GetAllSearchByUsername(IDbConnection dbClient, string searchData) => dbClient.Query<RoomEntity>(
        ...
        room.users_now, room.allow_hidewireds, room.price, room.wired_security, room.tile_flow, user.id as owner_id, user.langue
        FROM room
    ...
    internal static List<RoomEntity> GetAllSearch(IDbConnection dbClient, string searchData) => dbClient.Query<RoomEntity>(
        ...
        room.`wired_security`, room.`tile_flow`, user.`id` as `owner_id`, user.`langue`
        FROM `room`
        LEFT JOIN user ON room.owner = user.username
