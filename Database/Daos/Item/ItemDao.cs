...
internal sealed class ItemDao
{
    ...
    internal static void UpdateIsBC(IDbConnection dbClient, int itemId, bool isBc) => dbClient.Execute(
        "UPDATE item SET is_bc = @IsBc WHERE id = @ItemId LIMIT 1",
        new { IsBc = isBc, ItemId = itemId });

    internal static void UpdateColours(IDbConnection dbClient, int itemId, string[] colours) => dbClient.Execute(
        "UPDATE item SET colour_1 = @Colour1, colour_2, @Colour2 WHERE id = @ItemId LIMIT 1",
        new { Colour1 = colours[0], Colour2 = colours[1], ItemId = itemId });
