...
internal sealed class PurchaseFromCatalogEvent : IPacketEvent
{
    ...
    public void Parse(GameClient session, ClientPacket packet)
    {
        ...
        if (!string.IsNullOrEmpty(item.Badge) && !session.User.BadgeComponent.HasBadge(item.Badge))
        {
            ...
            if (item.Badge.Equals("DEC"))
            {
                if (session.User.Kisses >= 24)
                {
                    session.SendPacket(RoomNotificationComposer.SendBubble("error",
                        "Você atingiu o limite de acumulo de beijos!"));
                    session.SendPacket(new PurchaseErrorComposer());
                    return;
                }

                session.User.Kisses += item.Amount;
                session.SendPacket(new UserObjectComposer(session.User));
                session.SendPacket(RoomNotificationComposer.SendBubble("DEC", $"Foram comprados {item.Amount} beijos!"));
