...
public class ClientPacket(int messageID, byte[] body)
{
    ...
    public int RemainingLength => body.Length - this._pointer;
    ...
    {
        if (this.RemainingLength > 0 && body[this._pointer++] == Convert.ToChar(1))
        {
            return true;
        }
    ...
    public override string ToString() => "[" + this.Header + "] BODY: " + this._encoding.GetString(body).Replace(Convert.ToChar(0).ToString(), "[0]");
